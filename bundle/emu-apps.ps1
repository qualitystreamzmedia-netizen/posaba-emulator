...
# screen, next to Posaba TV. The portable emulator always boots -no-snapshot,
# which wipes every app except Posaba TV (which _run.cmd reinstalls); once
# userdata-qemu.img exists it persists, so most of this is a no-op after the
# first launch - but it self-heals if not.
#
...
# ---- 0. keep it minimal: no Play Store stub / Google search box ------------
foreach ($pkg in @('com.android.vending','com.google.android.googlequicksearchbox')) {
  & $Adb -s $Serial shell pm disable-user --user 0 $pkg *> $null
}
...
$calc = Join-Path $appsD 'posaba.apk'
if (Test-Path $calc) { & $Adb -s $Serial install -r $calc | Out-Null }   # -r so a newer build lands
...
DELETE FROM favorites WHERE intent LIKE '%com.google.android.gm/%' OR intent LIKE '%com.google.android.apps.photos/%' OR intent LIKE '%com.google.android.youtube/%' OR intent LIKE '%com.google.android.apps.messaging/%';
...
# ---- 2b. final sweep: a display-size change (emu-window "big") makes the
# launcher spawn a fresh 0-byte / root-owned grid DB; switching to that grid
# then crashes NexusLauncher to a black screen. Replace any such DB with the
# good one and force a clean launcher reload. ----------------------------
$good = $liveDbs | Where-Object { $n = FavCount $_; $n -ne '' -and [int]$n -ge 6 } | Select-Object -First 1
if ($good) {
  foreach ($f in ((Sh "ls $dbDir") -split '\s+' | Where-Object { $_ -match '^launcher.*\.db$' })) {
    if ($f -eq $good) { continue }
    $st = ((Sh "stat -c '%U %s' $dbDir/$f") -join '').Trim() -split '\s+'
    if ($st.Count -lt 2 -or $st[0] -ne $owner -or $st[1] -eq '0') {
      Sh "sh -c 'cp $dbDir/$good $dbDir/$f; rm -f $dbDir/$f-journal $dbDir/$f-wal $dbDir/$f-shm'" | Out-Null
      Sh "chown ${owner}:${owner} $dbDir/$f" | Out-Null
      Sh "chmod 660 $dbDir/$f" | Out-Null
    }
  }
}
Sh "pm disable-user com.google.android.apps.nexuslauncher" | Out-Null
Start-Sleep -Milliseconds 400
Sh "pm enable com.google.android.apps.nexuslauncher" | Out-Null
Start-Sleep -Milliseconds 700

# ---- 3. bump the on-device font a little ---------------------------------
# Scales Android system UI text + the Posaba apps (they use sp sizes). Retry
